...
## @brief weight conversion script for XLM-RoBERTa embedding model
...
def resolve_prefix(params):
    """Resolve the XLM-RoBERTa backbone key prefix inside the state dict.

    ``AutoModel`` (XLMRobertaModel) yields ``embeddings.word_embeddings.weight``;
    ``XLMRobertaForMaskedLM`` yields ``roberta.embeddings.word_embeddings.weight``.
    """
    for prefix in ["", "roberta."]:
        if f"{prefix}embeddings.word_embeddings.weight" in params:
            return prefix

    raise KeyError("Cannot find XLM-RoBERTa encoder weights in state_dict")


def _entries(params, n_layers):
    """Yield (nntrainer_name, hf_key, transpose) in nntrainer weight order.

    Mirrors BertTransformer in models/bert/bert_transformer.cpp (same graph
    structure as BERT).  Key differences from BERT:
      - type_vocab_size=1 (token_type_embeddings shape: [1, hidden])
      - layer_norm_eps=1e-5

    nntrainer weight order:
      word_emb, position_emb, token_type_emb, embedding LayerNorm(gamma, beta),
      per encoder layer: Wq(w,b) Wk(w,b) Wv(w,b) Wo(w,b) attn-LN(g,b)
                         ffn_fc1(w,b) ffn_down(w,b) ffn-LN(g,b)
    FC weights are transposed ([out,in] -> [in,out]); embeddings and
    LayerNorm params are stored as-is.
    """
...
def save_xlm_roberta_for_nntrainer(params, n_layers, dtype, file):
    """Convert and save weights as nntrainer binary format for XLM-RoBERTa."""
    for _, hf_key, transpose in _entries(params, n_layers):
        arr = tensor_to_numpy(params[hf_key], dtype, transpose=transpose)
        arr.tofile(file)


def collect_xlm_roberta_for_nntrainer(params, n_layers, dtype):
...
def parse_args():
    ...
    parser.add_argument(
        "--output_name",
        type=str,
        default="./nntr_xlm_roberta_fp32.bin",
...
def main():
    """Convert XLM-RoBERTa Hugging Face weights to nntrainer weight format."""
    args = parse_args()
    ...
    if args.safetensors:
        ...
        weights = collect_xlm_roberta_for_nntrainer(
    ...
    with open(args.output_name, "wb") as output_file:
        save_xlm_roberta_for_nntrainer(
